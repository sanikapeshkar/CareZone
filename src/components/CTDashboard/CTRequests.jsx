...
const CTRequests = () => {
  const { state, fetchPendingRequests, updateStatusForCustomer } = useContext(CTContext);
          ...
          {state.pendingRequests.map((val, key) => (
            <div key={key} className='grid grid-cols-5 w-full justify-between items-center mt-12 px-8'>
              ...
              <div className='flex flex-col items-end'>
                <button className='bg-green-500 text-white p-1 px-3 font-semibold rounded-2 w-[100px]' onClick={() => handleStatusUpdate(val.appointmentId, 'accepted')}>Accept</button>
                <button className='bg-red-500 text-white p-1 px-3 font-semibold rounded-2 mt-2 w-[100px]' onClick={() => handleStatusUpdate(val.appointmentId, 'rejected')}>Reject</button>
              </div>
            </div>
          ))}
        </div>
